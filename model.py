...
if selected_file != "Nenhum":
    st.write("### Classificação esperada:")
    ...
    try:
        ...
        # Exibir a classificação esperada para o arquivo selecionado
        st.write("### Classificação esperada:")
        ...
        # Gerar gráfico de barras com as quantidades de cada classe
        class_counts = pd.Series(y_pred_mapped).value_counts()
        fig, ax = plt.subplots()
        class_counts.plot(kind='bar', ax=ax)
        ax.set_xlabel("Classes")
        ax.set_ylabel("Quantidade de Previsões")
        ax.set_title("Distribuição das Previsões por Classe")
        st.pyplot(fig)
